-- Supplier Order Handling S4/S5: supplier accept/reject backend.
-- Forward-only migration. Does not connect supplier UI, preparation, delivery,
-- payments, commissions, settlements, withdrawals, refunds, or cancellations.

alter type public.order_status add value if not exists 'supplier_confirmed' after 'placed_pending_confirmation';
alter type public.order_status add value if not exists 'supplier_rejected' after 'supplier_confirmed';

alter table public.orders
  add column if not exists supplier_confirmed_at timestamptz,
  add column if not exists supplier_rejected_at timestamptz,
  add column if not exists supplier_rejection_reason_code text,
  add column if not exists supplier_rejection_reason_note text,
  add column if not exists supplier_decision_by_profile_id uuid references public.profiles(id) on delete set null,
  add column if not exists supplier_decision_idempotency_key text;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'orders_supplier_rejection_reason_code_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_supplier_rejection_reason_code_check
      check (
        supplier_rejection_reason_code is null
        or supplier_rejection_reason_code in (
          'out_of_stock',
          'product_unavailable',
          'unable_to_fulfil',
          'incorrect_listing',
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'orders_supplier_rejection_note_length_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_supplier_rejection_note_length_check
      check (supplier_rejection_reason_note is null or length(supplier_rejection_reason_note) <= 500);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'orders_supplier_decision_idempotency_key_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_supplier_decision_idempotency_key_check
      check (
        supplier_decision_idempotency_key is null
        or (
          length(trim(supplier_decision_idempotency_key)) > 0
          and length(supplier_decision_idempotency_key) <= 120
        )
      );
  end if;
end;
...
create or replace function public.supplier_order_decision_current_supplier_context()
returns table (
  profile_id uuid,
  supplier_id uuid,
...
begin
  ...
  return query select v_profile_id, v_supplier_id, v_actor_role;
end;
$$;
...
begin
  ...
  from public.supplier_order_decision_current_supplier_context()
  limit 1;
  ...
  if v_item.supplier_id <> v_context.supplier_id then
    raise exception 'ORDER_NOT_FOUND'
      using errcode = '42501';
  end if;
  ...
  if v_order.order_status <> 'placed_pending_confirmation'
     or v_order.payment_collection_status <> 'not_collected' then
    raise exception 'ORDER_NOT_ACTIONABLE'
      using errcode = '23514';
  end if;
  ...
  if v_reservation.reservation_status <> 'reserved' then
    raise exception 'RESERVATION_NOT_ACTIONABLE'
      using errcode = '23514';
  end if;
  ...
  set order_status = 'supplier_confirmed'::text::public.order_status,
      supplier_confirmed_at = coalesce(o.supplier_confirmed_at, now()),
      supplier_decision_by_profile_id = coalesce(o.supplier_decision_by_profile_id, v_context.profile_id),
      supplier_decision_idempotency_key = coalesce(o.supplier_decision_idempotency_key, v_idempotency_key),
      updated_at = now()
  where o.id = p_order_id
    and o.order_status = 'placed_pending_confirmation'
  ...
  if v_transitioned then
    ...
    values (
      ...
      jsonb_build_object('order_status', 'supplier_confirmed', 'supplier_id', v_context.supplier_id, 'idempotency_key_present', v_idempotency_key is not null),
      'Supplier accepted order; reservation and payment state preserved'
    );
  end if;
...
create or replace function public.supplier_reject_order(
  p_order_id uuid,
  p_reason_code text,
...
begin
  ...
  if v_variant.reserved_stock_quantity < v_reservation.quantity then
    raise exception 'RESERVED_STOCK_UNDERFLOW'
      using errcode = '23514';
  end if;

  update public.orders o
  set order_status = 'supplier_rejected'::text::public.order_status,
      supplier_rejected_at = coalesce(o.supplier_rejected_at, now()),
      supplier_rejection_reason_code = coalesce(o.supplier_rejection_reason_code, v_reason_code),
      supplier_rejection_reason_note = coalesce(o.supplier_rejection_reason_note, v_reason_note),
      supplier_decision_by_profile_id = coalesce(o.supplier_decision_by_profile_id, v_context.profile_id),
      supplier_decision_idempotency_key = coalesce(o.supplier_decision_idempotency_key, v_idempotency_key),
      updated_at = now()
  where o.id = p_order_id
  ...
  if v_transitioned then
    ...
    set reservation_status = 'released',
        ...
        release_reason = 'supplier_rejected:' || v_reason_code,
        updated_at = now()
    ...
    values (
      ...
      v_variant.total_stock_quantity,
      'supplier_rejected:' || v_reason_code,
      p_order_id,
      v_context.profile_id
    );
    ...
    values
      (
        ...
        p_order_id,
        jsonb_build_object('order_status', v_order.order_status::text),
        jsonb_build_object('order_status', 'supplier_rejected', 'reason_code', v_reason_code, 'supplier_id', v_context.supplier_id, 'idempotency_key_present', v_idempotency_key is not null),
        'Supplier rejected order with approved reason code'
      ),
      (
        v_context.profile_id,
        v_context.actor_role,
        ...
        'stock_reservations',
        v_reservation.id,
        jsonb_build_object('reservation_status', v_reservation.reservation_status::text),
        jsonb_build_object('reservation_status', 'released', 'reason_code', v_reason_code, 'quantity', v_reservation.quantity),
        'Supplier rejection released reserved stock'
      ),
      (
        v_context.profile_id,
        v_context.actor_role,
        ...
        'product_variants',
        v_variant.id,
        jsonb_build_object('reserved_stock_quantity', v_variant.reserved_stock_quantity),
        jsonb_build_object('reserved_stock_quantity', v_variant.reserved_stock_quantity - v_reservation.quantity, 'quantity', v_reservation.quantity),
        'Supplier rejection decremented reserved stock once'
      );
  end if;
...
begin
  ...
  select
    ...
    case
      ...
      when o.order_status::text = 'supplier_preparing' then 'Preparing'
...
begin
  ...
  select
    ...
    case
      ...
      when o.order_status::text = 'supplier_preparing' then 'Supplier preparing'
      when o.order_status::text = 'ready_for_pickup_or_dispatch' then 'Ready for pickup or dispatch'
      when o.order_status::text = 'out_for_delivery' then 'Out for delivery'
      when o.order_status::text = 'delivered_payment_pending' then 'Delivered - payment pending'
      when o.order_status::text = 'payment_collected' then 'Payment collected'
...
revoke all on function public.supplier_order_decision_current_supplier_context() from public;
revoke all on function public.supplier_order_decision_current_supplier_context() from anon;
revoke all on function public.supplier_order_decision_current_supplier_context() from authenticated;

revoke all on function public.supplier_accept_order(uuid, text) from public;
revoke all on function public.supplier_accept_order(uuid, text) from anon;
grant execute on function public.supplier_accept_order(uuid, text) to authenticated;

revoke all on function public.supplier_reject_order(uuid, text, text, text) from public;
revoke all on function public.supplier_reject_order(uuid, text, text, text) from anon;
grant execute on function public.supplier_reject_order(uuid, text, text, text) to authenticated;
...
comment on function public.supplier_accept_order(uuid, text)
  is 'Supplier-owner audited order acceptance boundary. Resolves supplier from the authenticated profile, locks order/item/reservation rows, preserves stock reservation and payment state, and returns the existing supplier-safe order detail shape.';

comment on function public.supplier_reject_order(uuid, text, text, text)
  is 'Supplier-owner audited order rejection boundary. Resolves supplier from the authenticated profile, locks order/item/reservation/variant rows, releases the active reservation exactly once, decrements reserved stock safely, and returns the existing supplier-safe order detail shape.';
