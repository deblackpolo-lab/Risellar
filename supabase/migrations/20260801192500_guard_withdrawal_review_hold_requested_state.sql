-- D9 forward patch: a withdrawal review hold must attach to an outstanding
-- requested withdrawal state. If payout wins the race first, finance must use
-- a liability/manual review path instead of creating a stale withdrawal hold.
...
drop trigger if exists finance_d9_withdrawal_review_hold_requested_guard on public.finance_holds;
create trigger finance_d9_withdrawal_review_hold_requested_guard
  before insert or update on public.finance_holds
  for each row
  execute function public.finance_d9_assert_withdrawal_review_hold_requested();
