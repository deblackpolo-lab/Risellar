...
-- Adds delivery_arranged to supplier order list status labels. This is read-only
-- and does not alter delivery, stock, payment, or finance behavior.
...
comment on function public.list_supplier_orders_safe(text, integer, timestamptz, uuid)
  is 'Supplier-owned Pay on Delivery order list including delivery_arranged display labels. Read-only and supplier-owner scoped.';