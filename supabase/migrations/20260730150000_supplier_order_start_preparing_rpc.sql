-- Supplier Fulfilment Phase 2: supplier start-preparing transition.
-- Forward-only migration. Does not implement delivery, payment collection,
-- rider assignment, commissions, settlements, withdrawals, refunds, or cancellations.

alter table public.orders
  add column if not exists supplier_preparing_at timestamptz,
  add column if not exists supplier_preparation_by_profile_id uuid references public.profiles(id) on delete set null,
  add column if not exists supplier_preparation_idempotency_key text;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'orders_supplier_preparation_idempotency_key_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_supplier_preparation_idempotency_key_check
      check (
        supplier_preparation_idempotency_key is null
        or (
          length(trim(supplier_preparation_idempotency_key)) > 0
          and length(supplier_preparation_idempotency_key) <= 120
        )
      );
  end if;
end;
...
create or replace function public.supplier_start_preparing(
  p_order_id uuid,
...
begin
  ...
  if v_order.order_status <> 'supplier_confirmed'
  ...
  set order_status = 'supplier_preparing'::text::public.order_status,
      supplier_preparing_at = coalesce(o.supplier_preparing_at, now()),
      supplier_preparation_by_profile_id = coalesce(o.supplier_preparation_by_profile_id, v_profile_id),
      supplier_preparation_idempotency_key = coalesce(o.supplier_preparation_idempotency_key, v_idempotency_key),
      updated_at = now()
  where o.id = p_order_id
    and o.order_status = 'supplier_confirmed'
  ...
  if v_transitioned then
    ...
    values (
      ...
      'Supplier started preparation; reservation, stock, payment, delivery, and finance state preserved'
    );
  end if;
...
revoke all on function public.supplier_start_preparing(uuid, text) from public;
revoke all on function public.supplier_start_preparing(uuid, text) from anon;
grant execute on function public.supplier_start_preparing(uuid, text) to authenticated;
...
comment on function public.supplier_start_preparing(uuid, text)
  is 'Supplier-owner audited start-preparing boundary. Emits supplier_order_preparation_started once, resolves supplier from the authenticated profile, locks order/item/reservation rows, preserves reservation, stock, payment, delivery, and finance state, and returns the existing supplier-safe order detail shape.';
