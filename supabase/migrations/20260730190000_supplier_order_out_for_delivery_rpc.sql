-- Delivery Phase 2: supplier marks an arranged Pay on Delivery order out for delivery.
-- Risellar records manual dispatch only. No provider booking, tracking, payment,
-- stock commit/release, commission, settlement, withdrawal, refund, cancellation,
-- delivered, or notification side effects are introduced here.

alter type public.order_status add value if not exists 'out_for_delivery';

alter table public.orders
  add column if not exists out_for_delivery_at timestamptz,
  add column if not exists out_for_delivery_by_profile_id uuid references public.profiles(id) on delete set null,
  add column if not exists out_for_delivery_idempotency_key text,
  add column if not exists dispatch_reference text,
  add column if not exists customer_dispatch_instruction text;

create index if not exists idx_orders_out_for_delivery_at
  on public.orders(out_for_delivery_at)
  where out_for_delivery_at is not null and deleted_at is null;
...
create or replace function public.supplier_mark_order_out_for_delivery(
  p_order_id uuid,
  p_dispatch_reference text default null,
  p_customer_dispatch_instruction text default null,
...
declare
  ...
  v_dispatch_reference text := nullif(trim(coalesce(p_dispatch_reference, '')), '');
  v_customer_dispatch_instruction text := nullif(trim(coalesce(p_customer_dispatch_instruction, '')), '');
...
begin
  ...
  if length(coalesce(v_dispatch_reference, '')) > 100
    or length(coalesce(v_customer_dispatch_instruction, '')) > 500 then
    raise exception 'FIELD_TOO_LONG'
      using errcode = '23514';
  end if;

  if coalesce(v_dispatch_reference, '') ~ '<[^>]+>'
    or coalesce(v_customer_dispatch_instruction, '') ~ '<[^>]+>' then
    raise exception 'INVALID_DISPATCH_FIELD'
      using errcode = '23514';
  end if;

  if concat_ws(' ', v_dispatch_reference, v_customer_dispatch_instruction) ~* '(gps|latitude|longitude|live tracking|verified by risellar|rider verified)' then
    raise exception 'INVALID_DISPATCH_FIELD'
      using errcode = '23514';
  end if;
  ...
  if v_order.order_status::text = 'out_for_delivery' then
    if v_order.out_for_delivery_idempotency_key is not distinct from v_idempotency_key
      and v_order.dispatch_reference is not distinct from v_dispatch_reference
      and v_order.customer_dispatch_instruction is not distinct from v_customer_dispatch_instruction then
  ...
  if v_order.order_status::text <> 'delivery_arranged' then
    if v_order.order_status::text = 'ready_for_delivery' then
      raise exception 'ORDER_NOT_ARRANGED'
        using errcode = '23514';
    end if;
  ...
  if v_order.delivery_arranged_at is null then
    raise exception 'ORDER_NOT_ARRANGED'
      using errcode = '23514';
  end if;
  ...
  set order_status = 'out_for_delivery'::text::public.order_status,
      ...
      out_for_delivery_by_profile_id = coalesce(o.out_for_delivery_by_profile_id, v_profile_id),
      out_for_delivery_idempotency_key = coalesce(o.out_for_delivery_idempotency_key, v_idempotency_key),
      dispatch_reference = coalesce(o.dispatch_reference, v_dispatch_reference),
      customer_dispatch_instruction = coalesce(o.customer_dispatch_instruction, v_customer_dispatch_instruction),
      updated_at = now()
  where o.id = p_order_id;
  ...
  where al.target_entity_type = 'orders'
    ...
    and al.action = 'supplier_order_out_for_delivery';
  ...
  if coalesce(v_existing_audit_count, 0) = 0 then
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'dispatch_reference_present', v_dispatch_reference is not null,
        'customer_dispatch_instruction_present', v_customer_dispatch_instruction is not null,
        'idempotency_key_present', v_idempotency_key is not null
      ),
      'Supplier marked manually arranged delivery as out for delivery outside Risellar'
    );
  end if;
...
comment on function public.supplier_mark_order_out_for_delivery(uuid, text, text, text)
  is 'Supplier-owner audited manual out-for-delivery boundary. Records dispatch only after delivery_arranged while preserving arrangement, reservation, stock, payment, and commercial snapshots.';
...
revoke all on function public.supplier_mark_order_out_for_delivery(uuid, text, text, text) from public, anon, authenticated;
grant execute on function public.supplier_mark_order_out_for_delivery(uuid, text, text, text) to authenticated;

comment on column public.orders.out_for_delivery_at
  is 'Manual out-for-delivery timestamp recorded by supplier after a delivery arrangement starts. Does not imply delivery, payment collection, provider booking, stock commit, settlement, commission, or withdrawal.';
comment on column public.orders.dispatch_reference
  is 'Optional supplier-entered manual dispatch reference. Not a provider API or tracking integration.';
comment on column public.orders.customer_dispatch_instruction
  is 'Optional customer-visible dispatch update entered by supplier. Must not claim Risellar rider verification or live tracking.';
